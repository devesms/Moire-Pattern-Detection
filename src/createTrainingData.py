...
def transformImageAndSave(image, f, customStr, path):
    ...
    fLL = (f.replace(fileName, fileName+'_' + customStr + 'LL')
           ).replace('.jpg', '.tiff')
    fLH = (f.replace(fileName, fileName+'_' + customStr + 'LH')
           ).replace('.jpg', '.tiff')
    fHL = (f.replace(fileName, fileName+'_' + customStr + 'HL')
           ).replace('.jpg', '.tiff')
    fHH = (f.replace(fileName, fileName+'_' + customStr + 'HH')
           ).replace('.jpg', '.tiff')
...
def createTrainingData(positiveImagePath, negativeImagePath):
    ...
    if not os.path.exists(positiveImagePath):
        print(f"Error: {positiveImagePath} does not exist!")
        return

    if not os.path.exists(negativeImagePath):
        print(f"Error: {negativeImagePath} does not exist!")
        return
    ...
    for f in positiveImageFiles:
        ret = augmentAndTrasformImage(
            f, positiveImagePath, positiveTrainImagePath)
        if ret == None:
            continue
        Kpositive += 3
    ...
    for f in negativeImageFiles:
        ret = augmentAndTrasformImage(
            f, negativeImagePath, negativeTrainImagePath)
        if ret == None:
            continue
        Knegative += 3
...
def parse_arguments(argv):
    ...
    parser.add_argument('positiveImages', type=str,
                        help='Directory with positive (Moiré pattern) images.')
    parser.add_argument('negativeImages', type=str,
                        help='Directory with negative (Normal) images.')
